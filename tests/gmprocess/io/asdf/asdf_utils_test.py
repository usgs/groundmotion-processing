...
def generate_workspace():
    ...
    quake_ml = h5.create_dataset(
        "QuakeML", data=np.ones(
            (512,), dtype='uint8'))

    waveforms = h5.create_group("Waveforms")
    st00 = waveforms.create_group("NET.ST00")
    st00.create_dataset(
        "NET.ST00.00.HN1__TSTART_TEND__EV0_label", data=np.ones(
            (300,), dtype='float64'))
    st00.create_dataset(
        "NET.ST00.00.HN2__TSTART_TEND__EV0_label", data=np.ones(
            (300,), dtype='float64'))
    st00.create_dataset(
        "NET.ST00.00.HNZ__TSTART_TEND__EV0_label", data=np.ones(
            (300,), dtype='float64'))
    st00.create_dataset(
        "NET.ST00.00.HN1__TSTART_TEND__EV1_label", data=np.ones(
            (400,), dtype='float64'))
    st00.create_dataset(
        "NET.ST00.00.HN2__TSTART_TEND__EV1_label", data=np.ones(
            (400,), dtype='float64'))
    st00.create_dataset(
        "NET.ST00.00.HNZ__TSTART_TEND__EV1_label", data=np.ones(
            (400,), dtype='float64'))
    st01 = waveforms.create_group("NET.ST01")
    st01.create_dataset(
        "NET.ST01.10.HNE__TSTART_TEND__EV0_label", data=np.ones(
            (300,), dtype='float64'))
    st01.create_dataset(
        "NET.ST01.10.HNN__TSTART_TEND__EV0_label", data=np.ones(
            (300,), dtype='float64'))
    st01.create_dataset(
        "NET.ST01.10.HNZ__TSTART_TEND__EV0_label", data=np.ones(
            (300,), dtype='float64'))
    ...
    station_metrics.create_dataset(
        "NET.ST00", data=np.ones(
            (100,), dtype='uint8'))
    station_metrics.create_dataset(
        "NET.ST01", data=np.ones(
            (200,), dtype='uint8'))

    waveform_metrics = aux_data.create_group("WaveformMetrics")
    waveform_metrics.create_dataset(
        "NET.ST00_EV0", data=np.ones(
            (100,), dtype='uint8'))
    waveform_metrics.create_dataset(
        "NET.ST00_EV1", data=np.ones(
            (200,), dtype='uint8'))
    waveform_metrics.create_dataset(
        "NET.ST01_EV0", data=np.ones(
            (300,), dtype='uint8'))

    processing_parameters = aux_data.create_group("TraceProcessingParameters")
    processing_parameters.create_dataset(
        "NET.ST00.00.HN1_EV0", data=np.ones(
            (100,), dtype='int32'))
    processing_parameters.create_dataset(
        "NET.ST00.00.HN1_EV1", data=np.ones(
            (200,), dtype='int32'))
    processing_parameters.create_dataset(
        "NET.ST01.10.HNE_EV0", data=np.ones(
            (300,), dtype='int32'))
...
def test_storage():
    ...
    total_bytes, groups = tally.compute_storage(
        h5.items(), store_subtotals=True)
    assert STORAGE == groups

    tally = TallyStorage()
    total_bytes, groups = tally.compute_storage(
        h5.items(), store_subtotals=False)
    total_bytes_storage = np.sum(
        [STORAGE[group]['total_bytes'] for group in STORAGE])
